...
if __name__ == '__main__':
    ...
    agent = TD3(env_refer, gamma, tau, buffer_maxlen, delay_step, noise_std, bound, critic_lr, actor_lr)
    # # 按照回合训练
    # max_episodes = 10000
    # max_steps = 1000
    # batch_size = 64
    ...
    for i in range(1000000):
        ...
        # print('the firiction is ',env_params)
        state = env.reset()
        episode_reward = 0
        for step in range(1000):
            ...
            if done or step == 1000 -1:
                t_episode_reward += episode_reward
                t_step += step
                # print('totle_step {}, episode_reward {}, step {}'.format(total_steps, episode_reward, step))
                break
            state = next_state
        print('totle_step {}, av_episode_reward {}, av_step {}'.format(total_steps, t_episode_reward, t_step))

        if i % 50 == 0:
            agent.save_model('TD3/td3_random.pth')
